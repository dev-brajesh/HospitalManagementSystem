...
<%
    ...
    String receptionistName = (String) session.getAttribute("fullName");
    if (receptionistName == null) receptionistName = "Receptionist";

    List<Visit> queue = null;
    try {
        queue = new VisitDAO().findUnassignedRegistrations();
    } catch (Exception e) {
        e.printStackTrace();
    }
%>
...
<div class="max-w-5xl mx-auto">
    ...
    <header class="bg-linear-to-r from-emerald-700 to-teal-600 text-white px-6 md:px-8 py-5 rounded-2xl shadow-xl">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div class="text-left md:text-right">
                <h2 class="text-2xl font-bold">Reception Dashboard</h2>
                <p class="text-emerald-100">Welcome, <%= receptionistName %></p>
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-emerald-200 overflow-hidden">
        <div class="bg-emerald-500 text-white px-6 py-4">
            <h2 class="text-xl font-bold">Registered Patient Queue</h2>
            <p class="text-emerald-100 text-sm">Patients waiting for doctor assignment</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tr>
                    ...
                    <th class="px-6 py-4 text-left">Patient ID</th>
                ...
                <tr><td colspan="5" class="p-6 text-center text-slate-400">No patients waiting</td></tr>
                <% } else {
                    int sno = 1;
                    for (Visit v : queue) { %>
                <tr class="border-t border-emerald-100">
                    <td class="px-6 py-4"><%= sno++ %></td>
                    <td class="px-6 py-4"><%= v.getPatientId() %></td>
